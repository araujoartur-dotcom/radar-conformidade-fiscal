...
export async function carregarCenariosDidaticosOficiais(empresaId: string): Promise<string[]> {
  ...
  // Grava as credenciais anotadas do CGIBS na tabela apuracao_credenciais_cgibs
  try {
    const db = getDatabase();
    const empRow = db.prepare('SELECT id FROM empresas WHERE id = ?').get(empresaId) as any;
    const targetEmpId = empRow ? empresaId : (db.prepare('SELECT id FROM empresas LIMIT 1').get() as any)?.id;
    if (targetEmpId) {
      db.prepare(`
        INSERT INTO apuracao_credenciais_cgibs (
          id, empresa_id, client_id, client_secret, token_contrib, status
        ) VALUES (?, ?, ?, ?, ?, ?)
      ...
      mensagens.push('🔐 Credenciais do CGIBS registradas com sucesso para a empresa ativa.');
    }
  } catch (err: any) {
    mensagens.push(`⚠️ Aviso ao salvar credenciais CGIBS: ${err.message}`);
  }
